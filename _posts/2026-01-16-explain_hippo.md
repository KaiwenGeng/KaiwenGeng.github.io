...
A hidden state $h_t \in \mathbb{R}^d$ usually attempts to perform two conflicting roles simultaneously:
...
Let $f(t)$ be the continuous input signal we want to memorize. We want to maintain a set of coefficients $c(t) \in \mathbb{R}^N$ that minimizes the approximation error between our polynomial estimate $g^{(t)}$ and the true history $f_{\le t}$ with respect to a specific measure (weighting) $\mu$.
...
Standard approaches (like the sliding window memory) require a fixed window size hyperparameter $\theta$.
...
Because the window stretches, the discretization becomes invariant to the step size $\Delta t$. When we discretize the ODE using the **Generalized Bilinear Transform (GBT)**, the step size $\Delta t$ effectively cancels out and is replaced by the step count $1/k$.
...
We construct a Recurrent Cell that follows a strict **Read $\to$ Think $\to$ Write** cycle for each time step $t$.
...
* $\tau$: A standard gated update function (like a GRU cell).
...
1.  **Gradient Flow (Proposition 5):**
    In HiPPO-LegS, the gradient norm decays as $\Theta(1/t)$. This is a **polynomial decay**, compared to the exponential decay ($e^{-t}$) of standard RNNs. This allows gradients to flow backward through thousands of steps, enabling the learning of very long-term dependencies.

2.  **Approximation Error (Proposition 6):**
    The error in memory reconstruction is bounded by $\mathcal{O}(1/\sqrt{N})$.
