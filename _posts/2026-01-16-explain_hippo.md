...
Let \\( f(t) \\) be the continuous input signal we want to memorize. We want to maintain a set of coefficients \\( c(t) \in \mathbb{R}^N \\) that minimizes the approximation error between our polynomial estimate and the true history.
...
We construct a Recurrent Cell that follows a strict **Read &rarr; Think &rarr; Write** cycle for each time step \\( t \\).
